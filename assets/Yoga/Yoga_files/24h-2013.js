var richZIndex=10000;var pageCookie=Math.floor(Math.random()*4);var bannerDisplay=new Array();if(c=getCookie('pageCookie')){pageCookie=parseInt(c);}
pageCookie=pageCookie%1000;function getElement(elementID){return document.getElementById(elementID);}
function isIE(){if(navigator.appName=='Microsoft Internet Explorer'){return true;}
return false;}
...
n_result=n_docel;return n_body&&n_body>300&&(!n_result||(n_result>n_body))?n_body:n_result;}
...
function expand(elementID,v_width1,v_height1,v_width2,v_height2,v_direction,v_type){headerObj=getElementsByClassName('header')[0];if(headerObj){headerObj.style.overflow='visible'}
...
this.width=width;this.height2=height2;this.width2=width2;this.type=type;this.name1=name1;this.name2=name2;this.desc=desc;};function genBanner(bannerArr,bannerClass){bannerClass=(bannerClass==undefined)?'m_banner_hide':bannerClass;str='';inlineCode='';bannerArr.richbanner=(bannerArr.width2>0&&bannerArr.height2>0)?true:false;if(bannerArr.richbanner){if(bannerArr.type=='lightbox'||bannerArr.type=='takeover'){str+='<div id="'+bannerArr.name+'" style="width:'+bannerArr.width+'px; height:'+bannerArr.height+'px;" class="'+bannerClass+'">';str+='</div>';bannerDisplay[bannerArr.name]=fw24h_getFloatFlash(bannerArr,'divID='+bannerArr.name+'&path='+bannerArr.bannerPath+'&bannerName='+bannerArr.name2+'&bannerWidth='+bannerArr.width2+'&bannerHeight='+bannerArr.height2+'&typeOpen='+bannerArr.type);}
...
else{bannerStr=new Array();bannerArr.aBanner=bannerArr.bannerPath.split('<|>');bWidth=(bannerArr.width>bannerArr.height&&bannerArr.aBanner.length>1)?bannerArr.width*2+5:bannerArr.width;for(i=0;i<bannerArr.aBanner.length;i++){if(i==0){bannerArr.bannerPath=bannerArr.aBanner[0];}
...
else{str+=genBanner(this.aNodes[iCtr],'m_banner_show');}}
document.write(str);str='';return str;};Banner.prototype.start=function(){return true;if(this.aNodes.length==0)
{return true;}
...
Banner.prototype.changeBanner=function(){try{var thisBanner;var prevBanner=-1;if(this.currentBanner>this.aNodes.length-1)
...
function d_Banner2(objName){this.obj=objName;this.aNodes=[];this.bNodes=[];this.currentBanner=0;};d_Banner2.prototype.add=function(bannerType,bannerPath,height,width,hyperlink,position,popup){var bannerDuration=0;this.aNodes[this.aNodes.length]=new Node(this.obj+"_"+this.aNodes.length,bannerType,bannerPath,bannerDuration,height,width,hyperlink,position,popup);};d_Banner2.prototype.add2=function(bannerType,bannerPath,height,width,hyperlink,position,popup){var bannerDuration=0;this.bNodes[this.bNodes.length]=new Node2(this.obj+"_b_"+this.bNodes.length,bannerType,bannerPath,bannerDuration,height,width,hyperlink,position,popup);};d_Banner2.prototype.add3=function(bannerType,bannerPath,bannerDuration,height,width,height2,width2,type,name1,name2,desc){var bannerDuration=0;this.aNodes[this.aNodes.length]=new NodeRich(this.obj+"_"+this.aNodes.length,bannerType,bannerPath,bannerDuration,height,width,height2,width2,type,name1,name2,desc);};d_Banner2.prototype.add4=function(bannerType,bannerPath,bannerDuration,height,width,height2,width2,type,name1,name2,desc){var bannerDuration=0;this.aNodes[this.bNodes.length]=new NodeRich(this.obj+"_"+this.aNodes.length,bannerType,bannerPath,bannerDuration,height,width,height2,width2,type,name1,name2,desc);};d_Banner2.prototype.toString=function(){var str="";var BannerPostion=pageCookie%this.aNodes.length;var i=1;for(var iCtr=BannerPostion;iCtr<this.aNodes.length;iCtr++){for(var iB=0;iB<this.bNodes.length;iB++){if(i==this.bNodes[iB].position){str+=genBanner(this.bNodes[iB],'d_banner2_show');i++;continue;}}
...
{cID=p_url.match(/cID=(\d*)&/);cID=cID[1];region=p_url.match(/region=([a-zA-z]*)&/);if(region){region=region[1];}else{region='HCM';}
static_domain=p_url.match(/static_domain=([^&]*)&/);if(static_domain){static_domain=static_domain[1];}else{static_domain='/';}
p_url=p_url.substring(p_url.indexOf('file=')+5);p_url=p_url.replace('&','');p_url=p_url.replace(' ','');urlArr=p_url.split(',');arr_playlist=new Array();for(i=0;i<urlArr.length;i++){arr_playlist[i]='{file:"'+urlArr[i]+'?start=0"';if(i==0){arr_playlist[i]+=(typeof p_before!='undefined'&&p_before!=''&&p_before!='/')?', image:"'+p_before+'"':', image:"/js/preview.swf"';}
arr_playlist[i]+='}';}
if(typeof p_end!='undefined'&&p_end!=''&&p_end!='/'){arr_playlist[urlArr.length]='{file:"'+p_end+'"}';}
v_playlist='['+arr_playlist.join()+']';playerID='mediaplayer'+Math.random();v_str_player='<div id="'+playerID+'"></div>';v_str_player+='<div id="'+playerID+'_ga" style="display:none"></div>';v_str_player+='<script type="text/javascript">';v_str_player+='jwplayer("'+playerID+'").setup({';v_str_player+='flashplayer: "/js/player.swf",';v_str_player+='playlist: '+v_playlist+',';v_str_player+='repeat: "list",';v_str_player+='stretching: "uniform",';v_str_player+='width: 528,';v_str_player+='height: 326,';v_str_player+='skin: "/js/trang-trong-24h.swf",';v_str_player+='"controlbar.idlehide":"true","controlbar.position":"bottom",';v_str_player+=(typeof p_corner!='undefined'&&p_corner!=''&&p_corner!='/')?'logo:"'+p_corner+'", "logo.hide":"false", "logo.position":"top-right", "logo.out":"1",':'';v_str_player+='"plugins": {"ova-jw": { "canFireEventAPICalls": true, "allowPlaylistControl": true, "overlays": { "closeButton": { "type": "custom", "image": "'+static_domain+'images/cross.png", "width": 15, "height": 15 } }, "ads": { "servers": [ { "type": "24h", "apiAddress": "'+static_domain+'upload/jsqcao/video_ad_content_'+cID+'_'+region+'.xml" } ],"schedule": [ { "zone": "28", "region": "auto:bottom", "width": 528, "height": 60, "duration": 15, "startTime": "00:00:10" } ] } },"gapro-2":{} },';v_str_player+='events: {onReady: function(event) {AjaxAction("'+playerID+'_ga", "/ajax/video_player_ga.html")}}';v_str_player+='})';v_str_player+='</script>';document.write(v_str_player);}
...
var bar='<img src="/images/loading.gif" align="absmiddle" alt="" /> &#272;ang t&#7843;i d&#7919; li&#7879;u';document.getElementById(where).innerHTML=bar
...
this.width=width;this.hyperlink=hyperlink;this.desc=desc;this.popup=popup;};Banner2.prototype.toString=function(){this.currentBanner=(pageCookie-1)%this.aNodes.length;var str=""
for(var iCtr=0;iCtr<this.aNodes.length;iCtr++){if(getCookie(this.aNodes[iCtr].name)){this.aNodes.splice(iCtr,1);continue;}
...
if(this.stopShow){return true;}
this.changeBanner();this.stopShow=true;var thisBannerObj=this.obj;return setTimeout(thisBannerObj+".start()",this.aNodes[this.currentBanner].bannerDuration*1000);}
...
if(this.currentBanner<this.aNodes.length-1){this.currentBanner=this.currentBanner+1;}else{this.currentBanner=0;}}
for(ii=0;ii<this.aNodes.length;ii++){if(document.getElementById(this.aNodes[ii].name)){document.getElementById(this.aNodes[ii].name).className="m_banner_hide";document.getElementById(this.obj+"_desc_"+ii).className="m_banner_lost_focus topbanner-text";}}
if(prevBanner>=0){document.getElementById(this.aNodes[prevBanner].name).className="m_banner_hide";document.getElementById(this.obj+"_desc_"+prevBanner).className="m_banner_lost_focus topbanner-text";}
document.getElementById(this.aNodes[thisBanner].name).className="m_banner_show";document.getElementById(this.obj+"_desc_"+thisBanner).className="m_banner_focus topbanner-text-active";}
var ddlevelsmenu={enableshim:true,hideinterval:200,effects:{enableswipe:true,enablefade:true,duration:100},httpsiframesrc:"blank.htm",topmenuids:[],topitems:{},subuls:{},lastactivesubul:{},topitemsindex:-1,ulindex:-1,hidetimers:{},shimadded:false,nonFF:!/Firefox[\/\s](\d+\.\d+)/.test(navigator.userAgent),getoffset:function(what,offsettype){return(what.offsetParent)?what[offsettype]+this.getoffset(what.offsetParent,offsettype):what[offsettype]},getoffsetof:function(el){el._offsets={left:this.getoffset(el,"offsetLeft"),top:this.getoffset(el,"offsetTop")}},getwindowsize:function(){this.docwidth=window.innerWidth?window.innerWidth-10:this.standardbody.clientWidth-10
...
var at_timeout=10;function at_show_aux(parent,child)
...
function at_hide()
{var c=document.getElementById(this["at_child"]);c.style.visibility="hidden";c["at_timeout"]=setTimeout("at_hide_aux('"+this["at_parent"]+"', '"+this["at_child"]+"')",at_timeout);}
...
{document.getElementById(parent).className="parent";document.write('<div class="vert_menu" id="'+parent+'_child" style="z-index:11000">');var n=0;for(var i in child)
{if(i=='-')
...
else{if(child[i].match('newwindow')){child[i]=child[i].replace(/newwindow:/,'javascript:');document.write('<a id="'+parent+'_'+n+'" href2="#" href="'+child[i]+'">'+i+'</a>');}else{document.write('<a id="'+parent+'_'+n+'" href="'+child[i]+'">'+i+'</a>');}}
n++;}
document.write('</div>');at_attach(parent,parent+"_child",position);}
function dhtmlmenu_build(menu)
{for(var i in menu)dhtmlmenu_build_aux(i,menu[i],"y");}
function indexTabOver(tab_id,tab_group){document.getElementById('tab1_'+tab_id).innerHTML='<img src="/images/index_box_01_04.gif" width="4" height="17" alt="" />';document.getElementById('tab3_'+tab_id).innerHTML='<img src="/images/index_box_01_06.gif" width="5" height="17" alt="" />';document.getElementById('tab4_'+tab_id).innerHTML='<img src="/images/index_box_01_10.gif" width="4" height="10" alt="" />';document.getElementById('tab5_'+tab_id).innerHTML='<img src="/images/index_box_01_11.gif" width="57" height="10" alt="">';document.getElementById("tab5_"+tab_id).style.background="url(/images/index_box_01_15.gif)";document.getElementById("tab2_"+tab_id).style.background="#80C141 url(/images/index_box_01_14.gif)";document.getElementById("tab2_"+tab_id).style.fontWeight='bold';document.getElementById("tab2_"+tab_id).style.color="#ffffff";document.getElementById("tab2_"+tab_id).className="home_index_tab_title_over";document.getElementById('tab6_'+tab_id).innerHTML='<img src="/images/index_box_01_12.gif" width="4" height="10" alt="" />';document.getElementById('tabContent_'+tab_id).style.display="block";}
...
for(var ii=0;ii<3;ii++){var descContent=(cBanner.aNodes[descPos[ii]]!=undefined)?cBanner.aNodes[descPos[ii]].desc:'';document.write('<td width="33%" class="topbanner-text" onclick="javascript:'+cBanner.obj+'.currentBanner='+descPos[ii]+';'+cBanner.obj+'.stopShow=true; '+cBanner.obj+'.changeBanner();" id="'+cBanner.obj+'_desc_'+descPos[ii]+'">'+descContent+'</td>');}
document.write('</tr></table>');}
function showeventdesc()
{var eventdiv=document.getElementById('eventdescid');var arrowimg=document.getElementById('eventarrow');var arrowimgup=document.getElementById('eventarrowup');eventdiv.style.display="block";arrowimg.style.display="none";arrowimgup.style.display="block";}
function hideeventdesc()
{var eventdiv=document.getElementById('eventdescid');var arrowimg=document.getElementById('eventarrow');var arrowimgup=document.getElementById('eventarrowup');eventdiv.style.display="none";arrowimg.style.display="block";arrowimgup.style.display="none";}
...
{tblObj=document.getElementById(elementID);tblObj.style.display=(tblObj.style.display=='none')?'':'none';}
function openVideo(phut)
{currentObj=document.getElementById('video_'+phut);if(!isIE()){currentObj.innerHTML=currentObj.innerHTML;currentObj.style.display=(currentObj.style.display=='none')?'':'none';btnObj=document.getElementById('xem_video_'+phut);btnObj.innerHTML=(btnObj.innerHTML=='Xem video')?'Đóng video':'Xem video';}else{currentObj.style.display='';}}
function seach_danh_ba_nha_hang(formSearch){var p_url=formSearch.url.value;var tinh=formSearch.Tinh.value;var quan=formSearch.Quan.value;var nhahangtype=formSearch.NhaHangType.value;var v_url=p_url.replace(/-c(\d*)t(\d*)p(\d*)q(\d*).html/i,'-c$1'+'t'+nhahangtype+'p'+tinh+'q'+quan+'.html');window.location=v_url;return false;}
...
function PopupWindow_refresh(){try{if(this.divName!=null){if(this.use_gebi){document.getElementById(this.divName).innerHTML=this.contents;}
...
if(trim(document.frmSendMail.Title.value)==""){document.frmSendMail.Title.value="Gui tin tu 24h qua email";}
...
function cookieSetListenPort(value){var cookieName='listen_port';setCookie(cookieName,value,24,'/','','');}
function _checkport(){try{var port_url='http://www.24h.com.vn'+':80'+'08/upload'+'/test_port.html';if(!window.shr)window.shr=new SWFHttpRequest();shr.open('GET',port_url,true);shr.onreadystatechange=function(){if(this.readyState!=4)return;if(this.status==200||this.status==206){var text=String(this.responseText);if(text.match(/^8008$/)){cookieSetListenPort('8008');}else{cookieSetListenPort('80');}}else{cookieSetListenPort('80');}};shr.send(null);}catch(e){}}
function changhomeimg(num)
{for(i=1;i<=num_news;i++){if(i!=num){document.getElementById('homeimg'+i).style.display='none';document.getElementById('home-sum-'+i).style.display='none';document.getElementById('numlink'+i).className='';}
document.getElementById('homeimg'+num).style.display='';document.getElementById('home-sum-'+num).style.display='';document.getElementById('numlink'+num).className='active';}}
function autochangeimg()
{if(cur_news==1){cur_news=2;}else{cur_news=1;}
changhomeimg(cur_news);setTimeout('autochangeimg()',10000);}
function tim_kiem_poll(p_url,p_input_id){if(document.getElementById(p_input_id)){var str_content=document.getElementById(p_input_id).value;if(str_content.length>500){alert('Nội dung tìm kiếm không được dài quá 500 ký tự');document.getElementById(p_input_id).focus();document.getElementById(p_input_id).select();return false;}
window.location=p_url;return true;}}
function reload_box_poll_khac(y,m,d)
{var v_date='today';d=parseInt(d);m=parseInt(m);if(!isNaN(y)){m=(m>=10)?m:'0'+m;d=(d>=10)?d:'0'+d;v_date=y+'-'+m+'-'+d;}
if(typeof(window.PARAMS)!="undefined"&&window.PARAMS!=null){v_div_id=window.PARAMS[0];v_goto_url=window.PARAMS[1];v_goto_url=v_goto_url.replace('{date}',v_date);if(v_goto_url!=''&&v_div_id!=''){AjaxAction(v_div_id,v_goto_url);}}}
function reload_url_gia_vang_trong_nuoc(y,m,d)
{d=d>=10?d:'0'+d;m=m>=10?m:'0'+m;window.location='/ttcb/giavang/gia-vang-ngay-'+d+'-thang-'+m+'-nam-'+y;}
function reload_url_gia_vang_quoc_te(y,m,d)
{d=d>=10?d:'0'+d;m=m>=10?m:'0'+m;window.location='/ttcb/giavang/gia-vang-ngay-'+d+'-thang-'+m+'-nam-'+y+'#gvqt';}
function xemtheongay_ban_tin_tai_chinh()
{var ddmmyyyy=document.ban_tin_tai_chinh.txtNgay2.value;var dd=ddmmyyyy.split("-");reload_ban_tin_tai_chinh(dd[2],dd[1],dd[0]);}
function reload_ban_tin_tai_chinh(y,m,d)
{var v_date='today';if(!isNaN(y)){m=(m>=10)?m:'0'+m;d=(d>=10)?d:'0'+d;v_date=y+'-'+m+'-'+d;}
if(typeof(window.PARAMS)!="undefined"&&window.PARAMS!=null){v_div_id=window.PARAMS[0];v_goto_url=window.PARAMS[1];v_goto_url=v_goto_url.replace('{date}',v_date);if(v_div_id!=''&&v_goto_url!=''){AjaxAction(v_div_id,v_goto_url);}}}
function reload_url_ty_gia(y,m,d)
{d=d>=10?d:'0'+d;m=m>=10?m:'0'+m;window.location='/ttcb/tygia/ty-gia-ngay-'+d+'-thang-'+m+'-nam-'+y;}
function reload_lich_truyen_hinh(y,m,d)
{var v_date='today';d=parseInt(d);m=parseInt(m);if(!isNaN(y)){m=(m>=10)?m:'0'+m;d=(d>=10)?d:'0'+d;v_date=y+'-'+m+'-'+d;}
if(typeof(window.PARAMS)!="undefined"&&window.PARAMS!=null){v_goto_url=window.PARAMS[0];v_goto_url=v_goto_url.replace('{date}',v_date);if(v_goto_url!=''){if(typeof(window.PARAMS[1])!="undefined"&&window.PARAMS[1]!=null){window.PARAMS[1]();}
window.location=v_goto_url;}}}
function xemtheongay_lich_truyen_hinh(p_form_name)
{var ddmmyyyy=document.forms[p_form_name].txtNgay2.value;var dd=ddmmyyyy.split("-");reload_lich_truyen_hinh(dd[2],dd[1],dd[0]);}
function tim_kiem_diem_dat_atm(p_goto_url,p_callback)
{var v_bank_id=document.getElementById("sel_bank").value;var v_city_id=document.getElementById("sel_city").value;var v_district_id=document.getElementById("sel_district").value;var v_street=document.getElementById("txt_address").value;var v_url=p_goto_url;v_url+="bank="+v_bank_id+"&city="+v_city_id+"&district="+v_district_id+"&street="+v_street;if(p_callback!=null&&p_callback!=''){p_callback();}
window.location=v_url;}
function isValidInputCommentATM(){v_email=trim(document.frmContact.txt_email.value);if(v_email==''){alert('Bạn chưa nhập địa chỉ email.');document.frmContact.txt_email.focus();return false;}
if(echeck(v_email)==false){alert('Địa chỉ email không chính xác.');document.frmContact.txt_email.focus();return false;}
if(v_email.length>100){alert('Email tối đa 100 ký tự.');document.frmContact.txt_email.focus();return false;}
v_body=trim(document.frmContact.txt_content.value);if(v_body==""){alert('Bạn chưa nhập nội dung.');document.frmContact.txt_content.focus();return false;}
if(v_body.length>5000){alert('Nội dung góp ý không được vượt quá 5000 ký tự.');document.frmContact.txt_content.focus();return false;}
v_ma_bao_mat=trim(document.frmContact.txt_ma_xac_minh.value);if(v_ma_bao_mat.length==0){alert('Bạn chưa nhập mã bảo mật');document.frmContact.txt_ma_xac_minh.focus();return false;}
document.frmContact.submit();return true;}